define(['./module'], function (directives) {
    ...
    directives.directive('loading', [function () {
        return {
            ...
            replace: true,
            scope: false,
            template: '<div id="loadding">' +
            '<div class="loading">' +
            '<img src="../app/image/loading.gif" style="width: 35px">' +
            '</div>' +
            '</div>'
        }
    }])
 ...
 //  模仿手机端滑动翻页
    directives.directive("myTabGroup",[function(){
        return {
            restrict:"EA",
            link:function(scope,ele,attr){
                ele = $(ele);
                ele.children('.tabGroup_items').children('[tab-group-item]').click(function(){
                    var th = $(this);
                    if(th.hasClass("active")){return ;}
                    var item = th.attr("tab-group-item");
                    var counts = ele.children('.tabGroup_counts').children("[tab-group-count="+item+"]");
                    th.addClass("active").siblings().removeClass('active');
                    counts.addClass("active tabContMove").siblings().removeClass("active tabContMove");

                    var sFn = counts.attr("data-fn");
                    if( !!sFn ){
                        var fn = eval('scope.'+sFn);
                        if( !!fn ){
                            fn();
                        }
                    }
    ...
    directives.directive('ngCisGridPager', function () {
        return {
            ...
            templateUrl: "./Templates/sharedTemplages/pagingTemplate.html",
            link: function ($scope, $element, $attr, ctrl) {
                var pageModule = $scope.pageModule = {
                    ...
                    lgPageSelectChange: function (size) {
                        $scope.ngCisGridPager.pageIndex = $scope.pageModule.pageIndex=$scope.pageModule.pageSet= 1;
    ...
    directives.directive('imgModelTemp',[function(){
        return {
            restrict:"A",
            replace:true,
            templateUrl:"Templates/sharedTemplages/img.html",
            link:function(scope,ele,attr){
